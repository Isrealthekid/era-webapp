  ...
  return (
    <section className="py-16 md:py-24">
      <div className="container mx-auto px-4">
        ...
        {featured && (
          <Link href={`/blog/${featured.slug}`} className="block rounded-2xl bg-card border border-border overflow-hidden hover-lift mb-10 max-w-5xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2">
              <div className="aspect-video md:aspect-auto bg-secondary relative min-h-[200px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {rest.map((b, i) => (
            <motion.div key={b.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
              <Link href={`/blog/${b.slug}`} className="block rounded-2xl bg-card border border-border overflow-hidden hover-lift group">
