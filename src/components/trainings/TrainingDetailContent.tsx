...
const TrainingDetailContent = ({ training }: TrainingDetailContentProps) => {
  ...
  return (
    <section className="py-12 md:py-20">
      <div className="container mx-auto px-4">
        <Link href="/trainings" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7">
            <div className="aspect-video rounded-2xl bg-secondary overflow-hidden mb-6 relative">
              <Image src={training.flyer} alt={training.title} fill className="object-cover" />
            </div>
      ...
      <Dialog open={enrollOpen} onOpenChange={setEnrollOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div><Label>Full Name</Label><Input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Your name" /></div>
          ...
          <DialogFooter>
            <Button onClick={handleEnroll} className="rounded-full">Submit Enrollment</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </section>
...
export default TrainingDetailContent;
